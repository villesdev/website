...
if len(assets) > 0:
    ...
    with goodorbad:
        if number_tickers == 1:
            st.title('\n')
            if change > 0.01:
                st.subheader(f"{assets} was a :green[good] investment, returning" + f"\$ :green[{multiplereturndollar:.2f}]")
            if change < 0.01:
                st.subheader(f"{assets} was a :red[bad] investment" + f"\$ :red[{multiplereturndollar:.2f}]")
        if number_tickers > 1:
            st.title('\n')
